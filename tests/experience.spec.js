...
test('scene loads, controls work, credits and sound respond',async({page},info)=>{
 ...
 await expect(page.locator('#world')).toHaveAttribute('data-actors','6');await expect(page.locator('#world')).toHaveAttribute('data-animated','6');
 ...
 if(info.project.name==='mobile'){await expect(page.locator('#joystick')).toBeVisible();const b=await page.locator('#joystick').boundingBox();expect(b.x).toBeGreaterThanOrEqual(0);}else{const before=await page.locator('#world').getAttribute('data-camera');await page.locator('#world').focus();await page.keyboard.down('KeyW');await page.waitForTimeout(800);await page.keyboard.up('KeyW');await expect(page.locator('#world')).not.toHaveAttribute('data-camera',before);}
