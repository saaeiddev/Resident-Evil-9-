...
export function surface(kind,repeat=1){
 ...
 if(kind==='brick'){for(let y=0;y<16;y++)for(let col=-1;col<8;col++){const bx=col*80+(y%2)*40,by=y*32;x.fillStyle=`rgb(${51+r()*16},${49+r()*14},${46+r()*12})`;x.fillRect(bx+2,by+2,76,28);x.strokeStyle='#353731';x.lineWidth=1;x.strokeRect(bx+2,by+2,76,28);}}
