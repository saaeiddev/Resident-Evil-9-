...
async function start(){
 ...
 if(input.touch){camera.fov=62;camera.updateProjectionMatrix();}
 ...
 let last=performance.now(),time=0;renderer.setAnimationLoop(now=>{if(document.hidden){last=now;return;}const wallDt=Math.max(0,(now-last)/1000),dt=Math.min(wallDt,.06);last=now;time+=wallDt;weatherFx.update(dt,time);city.update(time);cast.update(dt,time);
  if(introStart!==null){const f=Math.min((time-introStart)/12,1),s=f*f*(3-2*f);camera.position.set(T.MathUtils.lerp(5.8,input.touch?0:.8,s),T.MathUtils.lerp(2.4,1.68,s),T.MathUtils.lerp(9,input.touch?9:4.7,s));camera.lookAt(T.MathUtils.lerp(-1,-.7,s),1.05,T.MathUtils.lerp(-10,-4.5,s));if(f===1)finishIntro();}else if(!started){camera.position.x=5.8+Math.sin(time*.08)*.18;camera.lookAt(-1,1,-6);}else input.update(dt,time);
 ...
 function reset(){camera.position.set(input.touch?0:.8,1.68,input.touch?9:4.7);camera.lookAt(-.7,1.05,-4.5);finishIntro();}
