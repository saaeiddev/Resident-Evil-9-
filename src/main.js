...
async function start(){
 const canvas=$('world'),renderer=new T.WebGLRenderer({canvas,antialias:false,powerPreference:'high-performance'});renderer.setClearColor('#121b20');renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.4;renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;
 ...
 let last=performance.now(),time=0;renderer.setAnimationLoop(now=>{if(document.hidden){last=now;return;}const dt=Math.min((now-last)/1000,.06);last=now;time+=dt;weatherFx.update(dt,time);city.update(time);cast.update(dt,time);
  ...
  frameCount++;frameTime+=dt;if(frameTime>=2){fps=Math.round(frameCount/frameTime);$('stats').textContent=`${quality} · ${fps} FPS · ${renderer.info.render.calls} draw calls · ${Math.round(renderer.info.render.triangles/1000)}k triangles`;canvas.dataset.fps=String(fps);canvas.dataset.quality=quality;frameTime=frameCount=0;if(auto&&time-adaptAt>8&&fps<25&&quality!=='Low'){setQuality(quality==='Ultra'?'High':quality==='High'?'Medium':'Low');adaptAt=time;}}
 });
 ...
 $('attributions').innerHTML='Tactical character & animation: Adobe / Mixamo (royalty-free use in this interactive experience).<br>HVGirl: Babylon.js Assets contributors (CC BY 4.0; recolored for this scene).<br>City, motorcycle, surface textures & synthesized audio: original project assets.<br>Three.js: three.js authors (MIT).';
